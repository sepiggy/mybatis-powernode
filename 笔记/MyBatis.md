...
3） #{} 常常作为`列值`使用的， 位于等号的右侧， #{}位置的值和数据类型有关 (eg. pst.setInt(1,1001), 这里#{}占位符的类型就是int类型)。
...
4）${} 常用作`表名`或者`列名`， 在能保证数据安全的情况下使用 ${}占位符。
...
第一种方式： 在java程序中，把like的内容组装好。 把这个内容传入到sql语句。（推荐）
